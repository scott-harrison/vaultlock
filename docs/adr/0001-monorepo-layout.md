...
│   └── adr/
├── .github/
│   └── workflows/           # Shared CI (tests, coverage, security)
├── docker-compose.yml
├── TESTING.md
└── README.md
```
...
- Add root-level scripts: `npm run test:all`, `npm run coverage`.
- Enforce `TESTING.md` standards in every package from the first commit.
